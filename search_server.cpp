...
void SearchServer::AddDocument(int document_id, const string& document, DocumentStatus status, const vector<int>& ratings) {
    ...
    if (documents_.find(document_id) != documents_.end()) {
        throw invalid_argument("попытка добавить документ с id ранее добавленного документа!"s);
    }
    const vector<string> words = SplitIntoWordsNoStop(document); // проверка на спецсимволы перенесена
